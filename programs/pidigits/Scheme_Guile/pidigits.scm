...
(define (mk q r s t k) (comp q r s t k (* 2 (+ 1 (* 2 k))) 0 (+ 1 (* 2 k))))
            ...
            (let-values ([(q r s t) (prod q r s t y)])
                        (if (= col 10)
                            (let ([row (+ row 10)])
                              ...
                              (display "\n" out) (display y out)
                              (digit k q r s t (- n 1) row 1))
                            (begin
			      (display y out)
                              (digit k q r s t (- n 1) row (+ 1 col)))))
            (let-values ([(q r s t) (mk q r s t k)])
                        (digit (+ 1 k) q r s t n row col))))
      (begin
        (display (make-string (- 10 col) #\space) out) (display "\t:" out)
